...
export default function ProfitLossPage() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-3 py-4 text-xs">
      {/* Header */}
      <div className="mb-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            <h1 className="text-lg font-bold text-slate-800">📈 Laporan Laba / Rugi</h1>
      ...
      {/* Panduan Rumus */}
      <section className="mb-4 p-3 bg-blue-50 rounded border border-blue-200">
        <h3 className="font-medium text-blue-800 mb-1">ℹ️ Cara Menghitung Laba/Rugi</h3>
        <p className="text-blue-700">
          <strong>Rumus:</strong>{' '}
          <code className="font-mono bg-blue-100 px-1.5 py-0.5 rounded text-[10px]">
      ...
      <section className="mb-4">
        <h2 className="font-semibold text-slate-800 mb-2">📊 Ringkasan Keuangan</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-white p-3 rounded border border-slate-200 shadow-sm">
            <p className="text-slate-600">💰 Pendapatan</p>
            <p className="font-bold text-pink-600">Rp {summary.totalRevenue.toLocaleString()}</p>
          </div>
          <div className="bg-white p-3 rounded border border-slate-200 shadow-sm">
            <p className="text-slate-600">🧺 Biaya Bahan</p>
            <p className="font-bold text-amber-600">Rp {summary.totalMaterial.toLocaleString()}</p>
          </div>
          <div className="bg-white p-3 rounded border border-slate-200 shadow-sm">
            <p className="text-slate-600">🛠️ Biaya Lain</p>
            <p className="font-bold text-rose-600">Rp {summary.totalOther.toLocaleString()}</p>
          </div>
          <div className={`p-3 rounded border shadow-sm ${isProfit ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
            <p className="text-slate-600">{isProfit ? '✅ Laba' : '❌ Rugi'}</p>
            <p className={`font-bold ${isProfit ? 'text-emerald-700' : 'text-rose-700'}`}>
      ...
      {/* Filter */}
      <section className="p-3 bg-white rounded border border-slate-200 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
          <h2 className="font-semibold text-slate-800">🔍 Filter & Ekspor Data</h2>
          <button
            onClick={() => exportToCSV(filteredData)}
            disabled={filteredData.length === 0}
            className="px-2.5 py-1 text-[11px] font-medium rounded bg-slate-100 text-slate-700 hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
          >
            📤 Export ke CSV
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          <div>
            <label className="block text-slate-700 mb-1">Cari Produk</label>
            <input
              type="text"
              placeholder="Nama produk..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full px-2.5 py-1.5 text-[11px] border border-slate-300 rounded focus:ring-1 focus:ring-pink-200 outline-none"
            />
          </div>
          <div>
            <label className="block text-slate-700 mb-1">Tanggal Mulai</label>
            <input
              type="date"
          ...
          <div>
            <label className="block text-slate-700 mb-1">Tanggal Berakhir</label>
            <input
              type="date"
          ...
          <div>
            <label className="block text-slate-700 mb-1">Urutkan Berdasarkan</label>
            <select
              value={sort}
        ...
        <div className="bg-white rounded border border-slate-200 overflow-hidden shadow-sm">
          <div className="px-3 py-2 border-b">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
              ...
              <p className="text-slate-600">{filteredData.length} transaksi ditemukan</p>
            </div>
          </div>
          ...
          <div className="hidden sm:block overflow-auto max-h-[60vh]">
            <table className="min-w-full table-auto">
              <thead className="bg-pink-50 text-pink-700 sticky top-0 z-10">
                <tr>
                  <th className="p-1.5 text-left whitespace-nowrap">Tanggal</th>
                  <th className="p-1.5 text-left whitespace-nowrap">Produk</th>
                  <th className="p-1.5 text-center whitespace-nowrap">Qty</th>
                  <th className="p-1.5 text-left whitespace-nowrap">Pendapatan</th>
                  <th className="p-1.5 text-left whitespace-nowrap">Biaya Bahan</th>
                  <th className="p-1.5 text-left whitespace-nowrap">Biaya Lain</th>
                  <th className="p-1.5 text-left whitespace-nowrap">Laba</th>
                </tr>
              </thead>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item, idx) => (
                    <tr key={idx} className="hover:bg-pink-50">
                      <td className="p-1.5 whitespace-nowrap">{item.date}</td>
                      <td className="p-1.5 font-medium truncate max-w-[120px]" title={item.productName}>
                        {item.productName}
                      </td>
                      <td className="p-1.5 text-center whitespace-nowrap">{item.quantity}</td>
                      <td className="p-1.5 text-pink-600 font-medium whitespace-nowrap">Rp {item.revenue.toLocaleString()}</td>
                      <td className="p-1.5 text-amber-600 whitespace-nowrap">Rp {item.materialCost.toLocaleString()}</td>
                      <td className="p-1.5 text-rose-600 whitespace-nowrap">Rp {item.otherCost.toLocaleString()}</td>
                      <td className={`p-1.5 font-medium whitespace-nowrap ${item.profit >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
          ...
          {/* CARD (Mobile) */}
          <div className="block sm:hidden overflow-y-auto max-h-[60vh] space-y-2 p-2">
            {filteredData.length === 0 ? (
              ...
              filteredData.map((item, idx) => (
                <div
                  key={idx}
                  className="bg-white border border-slate-200 rounded p-2 shadow-sm hover:bg-slate-50"
                >
                  <div className="space-y-1">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-600">🛠️ Biaya Lain</span>
                      <span className="font-medium text-rose-600">Rp {item.otherCost.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">📈 Laba</span>
      ...
      {filteredData.length > 0 && (
        <section className="mt-4">
          <h2 className="font-semibold text-slate-800 mb-2">📈 Visualisasi Data</h2>
          <div className="space-y-3">
            {/* Grafik 1: Produk Paling Untung / Rugi */}
            <div className="bg-white p-3 rounded border border-slate-200 shadow-sm">
              <h3 className="font-medium text-slate-700 mb-1">Produk Paling Untung / Rugi</h3>
              <div className="w-full aspect-video max-h-[25vh] bg-gray-50 rounded">
            ...
            <div className="bg-white p-3 rounded border border-slate-200 shadow-sm">
              <h3 className="font-medium text-slate-700 mb-1">Pendapatan vs Biaya</h3>
              <div className="w-full aspect-video max-h-[25vh] bg-gray-50 rounded">
            ...
            <div className="bg-white p-3 rounded border border-slate-200 shadow-sm">
              <h3 className="font-medium text-slate-700 mb-1">Tren Laba Harian</h3>
              <div className="w-full aspect-video max-h-[25vh] bg-gray-50 rounded">
